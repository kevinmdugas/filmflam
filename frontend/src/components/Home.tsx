...
export const Home = () => {
	...
	function formatReview(review: Review): string {
		...
		if (review.addonStmt) {
			finalReview +=
				" " + review.addonStmt[0] + "  " + review.addonStmt[1] + "  " + review.addonStmt[2];
		}
		return finalReview;
	}

	const handleSaveReview = async () => {
		if (!review || !auth || !auth.user) return;
		const formattedReview = formatReview(review);
		if (!auth.user.reviews) auth.user.reviews = [formattedReview];
		else auth.user.reviews.push(formattedReview);
		try {
	...
	return (
		<>
			...
			<button onClick={onSubmitButtonClick} className="btn btn-primary my-3">
				Search for Title
			</button>
			{resTitles && !review && (
				<div>
					...
					{resTitles.length > 1 ? (
						...
						<p className="text-primary mb-4">
							Confirm this is the correct title to generate a new review!
						</p>
					)}
							...
							<div key={title.id} className="title-container">
								...
								<div>
									<span className="title-year">
										({title.year} {title.titleType == "movie" ? "Movie" : "TV Show"})
									</span>
								</div>
								<div>
								...
								<button onClick={() => handleTitleSelect(title)} className="btn btn-primary my-3">
									Select
								</button>
							</div>
						))}
				...
				<>
					...
					<div>
						...
						{auth?.user && !reviewSaved && (
							<button onClick={handleSaveReview} className="btn btn-primary m-3">
								Save Review
							</button>
						)}
						{auth?.user && reviewSaved && (
