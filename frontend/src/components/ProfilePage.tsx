...
export const ProfilePage = () => {
    ...
    if (!auth || !auth.user){
        return (
            <h1>INTRUDER ALERT! HALT, HACKER! I'M CALLING THE POLICE!</h1>
        )
    }
    return(
        <div className="container">
            <h1 className="text-title fs-1 fw-bold fst-italic mt-5 pt-3">{auth.user.name}'s Shame Chamber</h1>
            ...
            <div className="py-3 mt-3 card bg-body-secondary">
                <div className="card-body">
                    ...
                    <p className="card-text"><strong>Name:</strong> {auth.user.name}</p>
            ...
            <div className="card mt-3 py-3 bg-body-secondary">
                <div className="card-body">
                    ...
                    <p className="card-text"><strong>Favorite Film:</strong> {auth.user.favFilm}</p>
                    <p className="card-text"><strong>Favorite Actor:</strong> {auth.user.favActor}</p>
                    <p className="card-text"><strong>Favorite TV Show:</strong> {auth.user.favTVShow}</p>
                </div>
            </div>
